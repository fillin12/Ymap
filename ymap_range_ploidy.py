...
def Parse_File( input_file, chr_name, start_pos, end_pos ):
    ...
    for lines in input_file:
        ...
        string_indicator = String_Parser( lines_list[0], chr_name ) #looks for chromosome name in the line string
...
def Grand_File_Parser( input_file ):
    ...
    for lines in input_file:
        ...
        while ( i <= 7 ):
            ...
            if String_Parser( lines, chr_name_list[ i ] ) != []: #if the line contains information pertaining to the chromosome of choice
                ...
                chr_ploidy_dict[ chr_name_list[ i ] ][ 0 ] += float( line_list[ 5 ] )
                chr_ploidy_dict[ chr_name_list[ i ] ][ 1 ] += 1

            i += 1 

    return chr_ploidy_dict # Format will be [ strain_name, chr_1_ploidy, ... , chrR_ploidy ] 
...
def main():
    ...
    if len( arg_list ) == 1 or ( len( arg_list ) == 2 ): # Average ploidy of everything
        ...
        for files in file_list:
            ...
            print( new_line[:-2], file = output_file ) # [:-2] clips off the ", " at the very end of the string
        ...
        try:
            ...
            input_file_name  = arg_list[-4]
            chromosome_name  = arg_list[-3]
